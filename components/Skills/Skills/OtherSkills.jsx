...
export default function OtherSkills({ windowScreenWidth }) {
      ...
      return (
        ...
        >
          <div className="grid w-full grid-cols-1 py-3 sm:grid-cols-2 justify-evenly">
            {desc.map(({ name, color, href, by, id, date, icon }, index) => (
              <>
                <m.div
                  initial={{ opacity: 0, y: 5 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <div
                    className={`ml-2 text-sm font-medium text-gray-500 flex justify-center items-center ${
                      href ? `text-black text-sm text-start ` : ``
                    }`}
                  >
                        ...
                        {by ? (
                          <>
                            ...
                            >
                              ...
                              <div
                                className={`flex flex-col gap-1 ml-2 text-start`}
                              >
                                {by ? (
                                  <div className="text-sm tracking-tay-600 font-mediumfont-medium font-SourceCodePro">
                                    {by}
                                  </div>
                                ) : null}
                                {date ? (
                    ...
                    <a href={href}>
                      {href ? (
                        ...
                        >
                          {badgeTemplete(
                            "verify",
                            "text-red-400 border-red-400 hover:bg-red-400 hover:text-white active:bg-red-400 active:text-white"
                          )}
                        </a>
                      ) : (
                        ``
  ...
  const badgeTemplete = (content, ...args) => {
    switch (content) {
      case "expert":
        return (
          <div
            className={`p-1 ml-4 text-[.68rem] leading-3 sm:text-xs text-white lowercase border bg-red-400 rounded-md font-SourceCodePro  ${args}`}
          >
            expert
          </div>
        );
      case "verify":
        return (
          <div
            className={`p-1 ml-4 text-[.68rem] leading-3 sm:text-xs text-red-400 lowercase border border-red-400 rounded-md font-SourceCodePro  hover:bg-red-400 hover:text-white active:bg-red-400 active:text-white ${args}`}
          >
            Verify
          </div>
        );
      case "Cert.":
        return (
          <div
            className={`p-1 ml-4 text-[.68rem] leading-3 sm:text-xs text-purple-400 lowercase border border-purple-400 rounded-md font-SourceCodePro   ${args}`}
          >
            Cert.
          </div>
        );
      case "amature":
        return (
          <div
            className={`p-1 ml-4 text-[.68rem] leading-3 sm:text-xs text-blue-400 lowercase border border-blue-400 rounded-md font-SourceCodePro  ${args}`}
          >
            amature
          </div>
        );
      case "hands-on":
        return (
          <div
            className={`p-1 ml-4 text-[.68rem] leading-3 sm:text-xs text-green-500 lowercase border border-green-500 rounded-md font-SourceCodePro ${args}`}
          >
            hands-on
          </div>
        );
      case "intern":
        return (
          <div
            className={`p-1 ml-4 text-[.68rem] leading-3 sm:text-xs text-yellow-600 lowercase border border-yellow-600 rounded-md font-SourceCodePro ${args}`}
          >
            intern
          </div>
        );
      ...
      default:
        return (
          <div
            className={`flex justify-center items-center px-2 py-1 text-xs font-medium tracking-wide text-white rounded-full ${args}`}
          >
            expert
          </div>
        );
  ...
  const [otherSkills, setOtherSkills] = useState([
    {
      ...
      badges: ["expert", "intern", "Cert.", "hands-on"],
    },
    {
      name: "Networking",
      wordDesc:
        "Have been working with networking since 2018. I have worked with many networking devices and protocols. I have also worked with many network solutions like Packet Tracer and Wireshark.",
      desc: [
        {
          name: "Network Design",
          color: "bg-blue-500",
      ...
      badges: ["intern", "hands-on"],
    },
    {
      name: "Linux",
      wordDesc:
        "I have a good understanding of Linux and how it works. I have worked with many Linux distributions and I have also worked with many Linux tools.",
      icon: "https://cdn-icons-png.flaticon.com/512/2333/2333187.png",
      badges: ["hands-on", "amature"],
      desc: [
        {
...

    {
      name: "3D Designing",
      wordDesc:
        "I have been working as Freelance 3D Desginer since 2020. I have worked with many 3D designing tools and finished more than 150 projects.",
  ...
  return (
    <div className="grid items-start grid-cols-1 gap-4 sm:grid-cols-2">
      {otherSkills.map(
        (
          { name, desc, color, wordDesc, viewDesc, icon, badges, isOpen },
          index
        ) => (
          <m.div
            ...
            className={`${
              desc
                ? `cursor-pointer hover:shadow-lg ${
                    isOpen ? "sm:col-span-2 sm:shadow-lg" : ""
                  }`
                : "cursor-default"
            } border border-zinc-200/70 rounded-lg shadow-md`}
            onClick={() => {
              const newarray = update(otherSkills, {
                [index]: { isOpen: { $set: !isOpen } },
              });
              setOtherSkills(newarray);
            }}
          >
            <m.div className="flex items-center justify-between p-3 ">
              <div className="flex items-center flex-grow">
                <div
                  className={`${color}  ${
                    isOpen ? `mb-auto mt-[7.063px]` : `m-auto `
                  }`}
                >
                  <Image
                    width="1"
                ...
                <div className="w-full ml-4">
                  ...
                  {isOpen ? descTemplete(wordDesc, desc) : null}
                </div>
              </div>
              <div className={` rounded-lg p-1 `}>
                {desc || wordDesc ? (
                  <svg
                    ...
                    aria-hidden="true"
                    className={`${
                      viewDesc ? "rotate - 0" : "rotate-180"
                    } transform h-6 w-6 text-blue-500`}
                  >
                    <path
                      fill-rule="evenodd"
