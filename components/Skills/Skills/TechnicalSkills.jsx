...
export default function TechnicalSkills({ windowScreenWidth }) {
  ...
  return (
    <m.div initial={{ opacity: 0 }} animate={{ opacity: 1, type: "spring" }}>
      <div className="grid grid-cols-3 gap-3 sm:gap-4 sm:grid-cols-8 ">
        {skills
          .slice(0, handleSlice())
          .map(({ name, level, imgRounded, soon }, index) => (
            <m.div
              key={index}
              className={`flex flex-col justify-center px-1 py-2 bg-gray-100 rounded-lg  relative ${
                soon
                  ? `grayscale blur-[1.3px]  bg-zinc-200/80 shadow-none`
                  : `shadow-md`
              }`}
            >
              <div className="flex items-center justify-center flex-1">
                <Image
                  ...
                  alt={name}
                  className={`p-2  bg-gray-100 ${imgRounded} ${
                    soon ? `bg-gray-200` : `bg-gray-100`
                  }`}
                />
              </div>
              <div className="flex items-center justify-between w-full space-x-2">
                <div className="flex flex-col items-center justify-center w-full">
                  ...
                  <p
                    className={`text-xs font-SourceCodePro font-normal tracking-tight ${setColor(
                      level
                    )}`}
                  >
                    {level}
